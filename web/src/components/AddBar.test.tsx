...
const track = (id: string, thumbnailUrl: string | null = null): TrackMeta => ({
  videoId: id, title: `Title ${id}`, channel: "Chan", durationSec: 120, isLive: false, thumbnailUrl,
});
...
describe("AddBar", () => {
  ...
  it("BUG 6: renders the picker when candidates are present and queues a selected track", async () => {
  ...
  it("renders a graceful placeholder (no broken empty <img>) when thumbnailUrl is null", async () => {
    ...
    // (the previous `.every()` assertion was vacuously true on an empty NodeList).
  ...
  it("disables the input and button while busy (no voice channel target)", () => {
    render(<AddBar onPlay={async () => ({ candidates: null })} onQueueAll={async () => true} busy />);
